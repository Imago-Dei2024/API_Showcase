...
# ==== API Parameters ==== #
# 1. ❚ Required: function - The technical indicator of your choice. In this case, function=SMA
...
# 5. ❚ Required: time_period - Number of data points used to calculate each moving average value. Positive integers are accepted (e.g., time_period=60, time_period=200)
...
def sma_indicator():
    ...
    filename = f"Technical_Indicators/{symbol}_SMA_{time_period}_{interval}_{series_type}.csv"
    with open(filename, 'w') as file:
        file.write(response.text)

    print(f"Successfully Downloaded SMA Indicator for {symbol} to {filename}")
    ...
    # Display recent data using Rich with SMA interpretation
    if not df.empty:
        console.print(f"\n[bold green]Recent SMA({time_period}) Data for {symbol} ({interval} {series_type}):[/bold green]")
        ...
        for index, row in recent_data.iterrows():
            ...
            sma_value = row.get(f'SMA_{time_period}', 'N/A')
            
            # Simple trend analysis (compare with previous value if available)
            trend_indicator = ""
            if index > 0:
                prev_value = df.iloc[index-1].get(f'SMA_{time_period}', None)
                if prev_value is not None and sma_value != 'N/A':
                    try:
                        current_val = float(sma_value)
                        prev_val = float(prev_value)
                        if current_val > prev_val:
                            trend_indicator = "[green]↗ (Rising)[/green]"
                        elif current_val < prev_val:
                            trend_indicator = "[red]↘ (Falling)[/red]"
                        else:
                            trend_indicator = "[yellow]→ (Flat)[/yellow]"
                    except ValueError:
                        pass
            
            # Format the SMA value
            if sma_value != 'N/A':
                try:
                    sma_float = float(sma_value)
                    sma_formatted = f"{sma_float:.2f}"
                    console.print(f"[cyan]{date}:[/cyan] {sma_formatted} {trend_indicator}")
                except ValueError:
                    console.print(f"[cyan]{date}:[/cyan] {sma_value} {trend_indicator}")
            else:
                console.print(f"[cyan]{date}:[/cyan] {sma_value}")
        
        # Add interpretation note
        console.print(f"\n[bold yellow]Note:[/bold yellow] Rising SMA indicates uptrend, falling SMA indicates downtrend. Price above SMA suggests bullish momentum.")
