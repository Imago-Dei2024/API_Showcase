...
# ==== API Parameters ==== #
# 1. ❚ Required: function - The technical indicator of your choice. In this case, function=T3
...
def t3_indicator():
    ...
    while True:
        try:
            time_period = int(input("Enter Time Period (e.g., 20, 50, 200): "))
            if time_period > 0:
                break
            else:
    ...
    filename = f"Technical_Indicators/{symbol}_T3_{time_period}_{interval}_{series_type}.csv"
    with open(filename, 'w') as file:
        file.write(response.text)

    print(f"Successfully Downloaded T3 Indicator for {symbol} to {filename}")
